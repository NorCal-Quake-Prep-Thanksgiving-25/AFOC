...
try:  # Security: prefer GitPython to avoid shell invocation vulnerabilities.
    from git import GitCommandError, Repo
except Exception:  # pragma: no cover - fallback when GitPython unavailable
...
def _run_git_config(repo_root: Path, key: str, value: str) -> None:
    if Repo is None:
        logging.getLogger("GitSecurity").warning("GitPython not installed; skipping %s", key)
        return
    try:
        repo = Repo(repo_root)
