...
import hashlib  # Security: deterministic hashes replace pseudo-random seeding in simulations.
import re
from dataclasses import dataclass
from datetime import date, timedelta
import os
from random import SystemRandom  # Security: SystemRandom provides entropy for retry jitter.
import time
from typing import Any, Callable, ClassVar, Dict, Iterable, List, TypeVar

import logging

from ..pydantic_compat import BaseModel, Field

try:  # pragma: no cover - optional dependency variation
...
class CostCollector(BaseModel):
    ...
    def _simulate(self, window: CloudSpendWindow) -> List[CloudSpendSample]:
        ...
        return [
            CloudSpendSample(
                ...
                service=service,
                amount=base
                + (self._hash_fraction(f"{self.provider}:{service}:{window.start}:{window.end}"))
                * 100.0
                * span,
                start=window.start,
                end=window.end,
    ...
    def _call_with_backoff(self, func: Callable[[], T], *, label: str) -> T:
        ...
        while True:
            try:
                ...
                if sleep_for:
                    jitter = _SYSTEM_RANDOM.uniform(0, sleep_for / 2)
    ...
    def _hash_fraction(label: str) -> float:
        ...
        )  # Security: deterministic fraction avoids runtime PRNG usage.
    ...
    def _is_retryable(self, exc: Exception) -> bool:
        ...
        try:  # pragma: no cover - optional dependency
            ...
            LOGGER.debug(
                "Botocore not available", extra={"error": str(import_exc)}
            )  # Security: logging preserves diagnostics while falling back to HTTP heuristics.
        ...
        try:  # pragma: no cover - optional dependency
            ...
            LOGGER.debug(
                "Google API core not available", extra={"error": str(import_exc)}
            )  # Security: logged skip avoids silent suppression of retry signals.
        return False
...
def _extract_value(row: Any, key: str, index: int) -> Any:
    if hasattr(row, "get"):
        try:
            ...
            LOGGER.debug("Row access failure", extra={"error": str(exc)})
    if hasattr(row, key):
