...
try:  # pragma: no cover - optional dependency
    import redis  # type: ignore[import-untyped]  # Security: optional redis client imported dynamically.
except Exception:  # pragma: no cover - fallback
        ...
        try:
            ...
            self._spend_table = Table(
                ...
                Column(
                    ...
                    primary_key=True,
                    index=True,
                ),  # Security: composite key enforces tenant isolation preventing cross-tenant overwrite collisions.
            ...
            logger.error(
                "Failed to initialise relational store",
                extra={"error": str(exc)},
            )  # Security: structured logging avoids invalid kwargs while recording failure details.
            self._engine = None
            self._Session = None
            self._metadata = None
    ...
    def _initialise_cache(self) -> None:
        ...
        try:
            ...
            logger.warning(
                ...
                extra={"error": str(exc)},
            )  # Security: structured logging avoids invalid kwargs while preserving diagnostics.
            self._redis = None
            ...
            while attempts < max_attempts and not stored:
                ...
                try:
                    ...
                    if attempts >= max_attempts:
                        ...
                        )  # Security: structured logging ensures safe metadata propagation.
    ...
    def _initialise_fernet(self, key: bytes | None) -> Any:
        ...
        try:
            ...
            logger.error(
                ...
                extra={"error": str(exc)},
            )  # Security: structured logging avoids invalid kwargs while reporting key issues.
            return None
