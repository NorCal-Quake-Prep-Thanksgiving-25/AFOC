...
from ...telemetry.events import RightsizeEvent, Severity
...
def recommend(payload: RightsizingRequest) -> List[dict]:
    ...
    for rec in recommendations:
        ...
        publish(
            RightsizeEvent(
                scope=str(rec.get("resource_id", "unknown")),
                severity=Severity.INFO,
                payload={**rec, "savings": savings, "savings_ratio": ratio},
                dedupe_key=f"rightsize:{rec.get('resource_id')}:{date.today().isoformat()}",
            )
        )
