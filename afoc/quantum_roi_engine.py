...
try:  # Security: optional import enables genuine PQ primitives when library present.
    from pqcrypto.kem.kyber512 import generate_keypair as kyber_keypair
    from pqcrypto.sign.dilithium2 import generate_keypair as dilithium_keypair
except Exception:  # pragma: no cover - default to deterministic fallback
    kyber_keypair = None  # type: ignore
    dilithium_keypair = None  # type: ignore
...
class QuantumROIEngine:
    ...
    def post_quantum_signature(self) -> Mapping[str, str]:
        # Security: exposes PQ materials so downstream systems can combine classical and PQ credentials during audits.
        if kyber_keypair and dilithium_keypair:
            pk_enc, _ = kyber_keypair()
            pk_sig, _ = dilithium_keypair()
            return {
                "kyber_public": pk_enc.hex(),
                "dilithium_public": pk_sig.hex(),
                "note": "Hybrid PQ materials generated",
            }
        return {
            "kyber_public": "simulated",
            "dilithium_public": "simulated",
            "note": "PQ libraries unavailable; fallback material used",  # Security: explicit note prevents false sense of PQ assurance during audits.
        }
