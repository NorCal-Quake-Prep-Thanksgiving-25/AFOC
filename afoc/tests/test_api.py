...
try:  # pragma: no cover - optional dependency guard
    ...
    from afoc.telemetry.bus import publish
    from afoc.telemetry.events import (
        AnomalyEvent,
        ForecastEvent,
        RightsizeEvent,
        Severity,
...
def test_proof_endpoint_returns_payload(api_client: "TestClient") -> None:
    ...
    response = api_client.get(
        "/proof",
        params={"tier": "mid", "annual_spend": 2_000_000, "period": "last_30d"},
    )
    assert response.status_code == 200
    payload = response.json()
    assert "payload" in payload["json"]
    assert "markdown" in payload
    from afoc.db import session_scope
    from afoc.db.models import ValueProof

    with session_scope() as session:
        assert session.query(ValueProof).count() >= 1


@pytest.mark.skipif(
    TestClient is None or create_app is None or publish is None,
    reason="FastAPI not available",
)
def test_proof_endpoint_coerces_numeric_payloads(api_client: "TestClient") -> None:
    """String payload values should be coerced without raising."""

    publish(  # type: ignore[misc]
        AnomalyEvent(
            scope="acct/demo",
            severity=Severity.WARN,
            payload={"observed": "$125.40"},
            dedupe_key="anomaly:demo:test",
        )
    )
    publish(  # type: ignore[misc]
        RightsizeEvent(
            scope="i-123",
            payload={"savings": "45.5", "resource_id": "i-123"},
            dedupe_key="rightsize:i-123",
        )
    )
    publish(  # type: ignore[misc]
        ForecastEvent(
            scope="acct/demo",
            payload={"points": [{"yhat": "33.3"}]},
            dedupe_key="forecast:acct/demo",
        )
    )

    response = api_client.get(
        "/proof",
        params={"tier": "large", "annual_spend": 1_000_000, "period": "last_30d"},
    )
    assert response.status_code == 200
    payload = response.json()["json"]["payload"]
    assert math.isfinite(payload["anomaly"]) and payload["anomaly"] > 0
    assert math.isfinite(payload["rightsize"]) and payload["rightsize"] >= 0
    assert math.isfinite(payload["forecast"]) and payload["forecast"] >= 0
